...
class KFold(object):
    ...
    def stratified_split(self, train_df, group_col):
        ...
        train_df[self.flag_name] = train_df[self.flag_name].sample(
...
def get_score(df, masks, rank_pred, code_df_valid):
    ...
    df = pd.concat([df[['id', 'cell_id', 'rank2']],
                   code_df_valid_tmp]).reset_index(drop=True)
    df = df.sort_values(by=['id', 'rank2'], ascending=True)
    res = df.groupby(by=['id'], sort=False, as_index=False)[
        'cell_id'].agg(list)
...
def get_sorted_test_df(df, extract_col, tokenizer, batch_size, cell_max_length=128):
    ...
    sorted_input_length = sort_df['input_lengths'].values
    batch_max_length = np.zeros_like(sorted_input_length)
    total_iter = len(sorted_input_length) // batch_size if len(
        sorted_input_length) % batch_size == 0 else (len(sorted_input_length) // batch_size) + 1
